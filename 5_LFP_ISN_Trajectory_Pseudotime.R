...
#NP_obj$Cell_labels[NP_obj$integrated_snn_res.2 %in% c("14")] <- "ISN_pre"
...
##
Idents(NP_obj) <- factor(NP_obj$Cell_labels, levels = c("KA'", "KA\"", "V2s", "OPC", "Zic+", "LFP", "ISN", "ERG", "NPC", "GLU IN", "V2-pre", "NPro1",  "NPro2", "NPro3", "NPro4", "MN", "MC", "KC"))
######################## publication ################################################
# ISN markers:
library(Seurat)
library(ggplot2)
...
p3 <- p2 +         
  ...
  guides(color = guide_legend(override.aes = list(size=5))) #设置legend中
...
##################################################
## LFP, ISN
NP_LFP_ISNpre_ISN_obj_sel <- subset(NP_obj, ISN_ISN_Pre_labels %in% c("LFP", "ISN_pre", "ISN"))

# Plot LFP, ISN_pre, ISN
pdf("~/LFP_ISN_DimPlot_split_Stage.pdf", width = 15 * 2.5, height = 12)
DimPlot(NP_LFP_ISNpre_ISN_obj_sel, label =F, group.by = "ISN_ISN_Pre_labels", pt.size = 2 , split.by = "orig.ident") + scale_color_manual(values = c("LFP" = '#57C3F3', "ISN_pre" = "#91D0BE", "ISN" = '#476D87')) + theme(text = element_text(size = 30), legend.key.size = unit(1, 'cm'), legend.text = element_text(size=30)) + ggtitle("")

dev.off()

png("~/LFP_ISN_DimPlot_split_Stage.png", width = 480 * 3.5, height = 480*1.5)
DimPlot(NP_LFP_ISNpre_ISN_obj_sel, label =F, group.by = "ISN_ISN_Pre_labels", pt.size = 2 , split.by = "orig.ident") + scale_color_manual(values = c("LFP" = '#57C3F3', "ISN_pre" = "#91D0BE", "ISN" = '#476D87')) + theme(text = element_text(size = 30), legend.key.size = unit(1, 'cm'), legend.text = element_text(size=30)) + ggtitle("")

dev.off()

png("~/LFP_ISN_Feature_split_Stage.png", width = 480 * 3.5, height = 480*3)
nkx2.9_plot <- FeaturePlot(NP_LFP_ISNpre_ISN_obj_sel, c("nkx2.9"), pt.size = 2 , split.by = "orig.ident") + theme(text = element_text(size = 30), legend.key.size = unit(1, 'cm'), legend.text = element_text(size=30)) + ggtitle("")
nkx2.2a_plot <- FeaturePlot(NP_LFP_ISNpre_ISN_obj_sel, c("nkx2.2a"), pt.size = 2 , split.by = "orig.ident") + theme(text = element_text(size = 30), legend.key.size = unit(1, 'cm'), legend.text = element_text(size=30)) + ggtitle("")
foxa_plot <- FeaturePlot(NP_LFP_ISNpre_ISN_obj_sel, c("foxa"), pt.size = 2 , split.by = "orig.ident") + theme(text = element_text(size = 30), legend.key.size = unit(1, 'cm'), legend.text = element_text(size=30)) + ggtitle("")
fev_plot <- FeaturePlot(NP_LFP_ISNpre_ISN_obj_sel, c("fev"), pt.size = 2 , split.by = "orig.ident") + theme(text = element_text(size = 30), legend.key.size = unit(1, 'cm'), legend.text = element_text(size=30)) + ggtitle("")
tph2_plot <- FeaturePlot(NP_LFP_ISNpre_ISN_obj_sel, c("tph2"), pt.size = 2 , split.by = "orig.ident") + theme(text = element_text(size = 30), legend.key.size = unit(1, 'cm'), legend.text = element_text(size=30)) + ggtitle("")
print(nkx2.9_plot + nkx2.2a_plot + foxa_plot + fev_plot + tph2_plot)
...
Idents(NP_LFP_ISNpre_ISN_obj_sel) = NP_LFP_ISNpre_ISN_obj_sel$ISN_ISN_Pre_labels 
...
NP_LFP_ISNpre_ISN_obj_sel$Cell_label_Stage <- paste(NP_LFP_ISNpre_ISN_obj_sel$ISN_ISN_Pre_labels, NP_LFP_ISNpre_ISN_obj_sel$orig.ident, sep = "_")

#saveRDS(NP_LFP_ISNpre_ISN_obj_sel, file = "~/NP_LFP_ISN_pre_ISN_filterlabel_LFP_ISN_new.rds")


NP_LFP_ISNpre_ISN_obj_sel2 <- subset(NP_LFP_ISNpre_ISN_obj_sel, Cell_label_Stage %in% names(table(NP_LFP_ISNpre_ISN_obj_sel$Cell_label_Stage))[!c(names(table(NP_LFP_ISNpre_ISN_obj_sel$Cell_label_Stage)) %in% c("LFP_2dpf","LFP_3dpf", "LFP_5dpf"))])
NP_LFP_ISNpre_ISN_obj_sel2$Cell_labels_New <- NP_LFP_ISNpre_ISN_obj_sel2$ISN_ISN_Pre_labels
...
#plot_pseudotime_heatmap(monocle_cds[c(df_clusters$gene_names[df_clusters$clusters == 4], df_clusters$gene_names[df_clusters$clusters == 2], df_clusters$gene_names[df_clusters$clusters == 1], df_clusters$gene_names[df_clusters$clusters == 3]), ],   show_rownames = F,num_clusters = 4,cluster_rows = F ,return_heatmap = T)
...
xx <- compareCluster(gcSample, fun="enrichKEGG", organism="dre", pvalueCutoff= 1, qvalueCutoff = 1)
...
xx_Go3_MF <- compareCluster(gcSample, fun="enrichGO", OrgDb = org.Dr.eg.db, ont='MF',pAdjustMethod = 'BH',pvalueCutoff = 0.05, qvalueCutoff = 0.05,keyType = 'ENTREZID' , readable = TRUE )

df_xx_Go3_MF <- data.frame(xx_Go3_MF)
Time_sig_genes_heatmap.Go_MF.list <- list()
for(i in sort(unique(as.character(df_xx_Go3_MF$Cluster)))){
 
    Time_sig_genes_heatmap.Go_MF.list[[i]] <- df_xx_Go3_MF[df_xx_Go3_MF$Cluster == i, ]

}
writexl::write_xlsx(Time_sig_genes_heatmap.Go_MF.list, "~/Time_sig_genes_heatmap_AllCluster_Go_MF.xlsx")

#
xx_Go3_CC <- compareCluster(gcSample, fun="enrichGO", OrgDb = org.Dr.eg.db, ont='CC',pAdjustMethod = 'BH',pvalueCutoff = 0.05, qvalueCutoff = 0.05,keyType = 'ENTREZID' , readable = TRUE )

df_xx_Go3_CC <- data.frame(xx_Go3_CC)
Time_sig_genes_heatmap.Go_CC.list <- list()
for(i in sort(unique(as.character(df_xx_Go3_CC$Cluster)))){
 
    Time_sig_genes_heatmap.Go_CC.list[[i]] <- df_xx_Go3_CC[df_xx_Go3_CC$Cluster == i, ]

}
writexl::write_xlsx(Time_sig_genes_heatmap.Go_CC.list, "~/Time_sig_genes_heatmap_AllCluster_Go_CC.xlsx")
...
#
#
p_Go_MF <- dotplot(xx_Go3_MF, showCategory = length(unique(data.frame(xx_Go3_MF)$Description))) 

p_Go_MF_1 <- p_Go_MF + theme(axis.text.x = element_text(angle = 45, 
                                    vjust = 0.5, hjust=0.5))

pdf("~/Time_sig_genes_heatmap_GO_MF.pdf", width = 15, height = 22)
print(p_Go_MF_1)

dev.off()

#
p_Go_CC <- dotplot(xx_Go3_CC, showCategory = length(unique(data.frame(xx_Go3_CC)$Description))) 

p_Go_CC_1 <- p_Go_CC + theme(axis.text.x = element_text(angle = 45, 
                                    vjust = 0.5, hjust=0.5))

pdf("~/Time_sig_genes_heatmap_GO_CC.pdf", width = 10, height = 32)
print(p_Go_CC_1)
...
xx_Reactome <- compareCluster(gcSample, fun="enrichPathway", organism = "zebrafish", pAdjustMethod = 'BH',pvalueCutoff = 0.05, qvalueCutoff = 0.05,  readable = TRUE )


df_xx_Reactome <- data.frame(xx_Reactome)
Time_sig_genes_heatmap.Reactome.list <- list()
for(i in sort(unique(as.character(df_xx_Reactome$Cluster)))){
 
    Time_sig_genes_heatmap.Reactome.list[[i]] <- df_xx_Reactome[df_xx_Reactome$Cluster == i, ]

}
writexl::write_xlsx(Time_sig_genes_heatmap.Reactome.list, "~/Time_sig_genes_heatmap_Reactome.xlsx")

#
p_Reactome <- dotplot(xx_Reactome, showCategory = length(unique(data.frame(xx_Reactome)$Description))) 

p_Reactome_1 <- p_Reactome + theme(axis.text.x = element_text(angle = 45, 
                                    vjust = 0.5, hjust=0.5))
pdf("~/Time_sig_genes_heatmap_Reactome.pdf", width = 20, height = 16*5)
print(p_Reactome_1)
...
p_Reactome <- dotplot(xx_Reactome, showCategory = 20) 

p_Reactome_1 <- p_Reactome + theme(axis.text.x = element_text(angle = 45, 
                                    vjust = 0.5, hjust=0.5))
pdf("~/Time_sig_genes_heatmap_Reactome_top20.pdf", width = 15, height = 16)
print(p_Reactome_1)
...
png("~/Publication_LFP_ISN_Trajectory_stage.png", width = 480*1.4)
plot_cell_trajectory(monocle_cds, color_by = "orig.ident", cell_size = 5) + scale_color_manual(values = c("#999999", "#333333", "#ff0000", "#00cdcd")) + theme(text = element_text(size = 30), legend.text = element_text(size=30)) + guides(colour = guide_legend(override.aes = list(size=5)))
dev.off()
#
#monocle_cds <- reduceDimension(monocle_cds, max_components = 2, reduction_method = 'DDRTree', sigma = 0.25)
